...
auto find_output_report_size(const uint8_t id) -> std::optional<uint8_t> {
...
auto find_optimal_report_size(const size_t size) -> ReportSize {
    ...
    return input_report_size_table.back();
}
} // namespace

auto parse_hid_report(BytesArray& buf, BytesRef ref) -> bool {
    unwrap(report, bytes_as<IAPHIDReport>(ref));
    unwrap(report_size, find_output_report_size(report.report_id));
